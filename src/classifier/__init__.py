...
class TypeClassifier:
  ...
  def decide(self, token: str) -> str:
    ...
    elif TypeClassifier.__is_ip_address__(token):
      return TypeClassifier.IP_ADDRESS
    elif TypeClassifier.__is_email__(token, self.tlds):
      return TypeClassifier.EMAIL
    elif TypeClassifier.__is_url__(token, self.tlds):
      return TypeClassifier.URL
    elif TypeClassifier.__is_number__(token):
      return TypeClassifier.NUMBER
    elif TypeClassifier.__is_alpha__(token):
      return TypeClassifier.ALPHA
  ...
  def __is_email__(token: str, tlds: list[str]) -> bool:
    i = token.find('@')
    return len(token[:max(i, 0)].replace(
        ' ', '')) > 0 and TypeClassifier.__is_url__(token[i + 1:], tlds)
  ...
  def classify(self, s: Union) -> list[Class]:
